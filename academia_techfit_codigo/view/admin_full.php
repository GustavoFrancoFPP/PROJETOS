    ...
    try {
        switch ($action) {
            ...
            case 'enviar_notificacao':
                ...
                $stmt = $conn->prepare("SELECT id_cliente FROM cliente WHERE status = 'ativo'");
                $stmt->execute();
                $clientes = $stmt->fetchAll(PDO::FETCH_ASSOC);
                
                // Inserir notificação para cada cliente
                $stmt = $conn->prepare("INSERT INTO notificacao (titulo, mensagem, tipo, id_cliente, prioridade, status) VALUES (?, ?, 'geral', ?, ?, 'não lida')");
                ...
                $mensagem = "Notificação enviada com sucesso para {$totalEnviadas} aluno(s)!";
                break;
        }
