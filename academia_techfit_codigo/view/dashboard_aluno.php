...
try {
    // Notificações não lidas para este cliente específico
    $stmt = $conn->prepare("
    ...
    // Buscar notificações recentes do cliente (últimas 5)
    $stmt = $conn->prepare("
        SELECT 
            id_notificacao,
            titulo,
...
<body>
                ...
                <?php if (!empty($notificacoesRecentes)): ?>
                    <ul class="item-list" style="max-height: 400px; overflow-y: auto;">
                        <?php foreach ($notificacoesRecentes as $notif): ?>
                            <li data-notif-id="<?php echo $notif['id_notificacao']; ?>" style="flex-direction: column; align-items: flex-start; padding: 12px 0; <?php echo $notif['status'] === 'não lida' ? 'cursor: pointer;' : ''; ?>">
    ...
    <script>
        ...
        // Função para marcar notificação como lida ao clicar
        document.querySelectorAll('.item-list li[data-notif-id]').forEach(function(item) {
            const notifElement = item.querySelector('.fas.fa-circle');
            if (notifElement) {
                // Só adiciona o evento se a notificação não foi lida
                item.addEventListener('click', async function() {
                    const idNotificacao = this.getAttribute('data-notif-id');
                    ...
                    try {
                        ...
                        if (resultado.sucesso) {
                            // Remove o indicador visual de não lida
                            notifElement.remove();
                            // Atualiza o contador após um pequeno delay
                            setTimeout(() => {
                                location.reload();
                            }, 300);
                        }
